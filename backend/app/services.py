...
class ComparisonService:
    ...
    def compare(self, observation_id_parts: dict, dataset_id: Optional[str] = None) -> ComparisonResult:
        ...
        if dataset_id and dataset_id in candidate_datasets:
            chosen_dataset_id = dataset_id
        else:
            chosen_dataset_id = sorted(candidate_datasets)[0]

        model_rows = [r for r in store.model_records
                      if r.dataset_id == chosen_dataset_id and r.variable == variable]
        if not model_rows:
